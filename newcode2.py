...
# Load MNIST data.
images, labels = FashionMNIST(path=os.path.join('..', '..', 'data', 'FashionMNIST'), download=True).get_train()
...
# Load MNIST data.
images, labels = FashionMNIST(path=os.path.join('..', '..', 'data', 'FashionMNIST'), download=True).get_test()
images = images.view(-1, 784)
images *= intensity
...
for i in range(n_test):
    ...
    # Add to spikes recording.
    spike_record[i] = spikes['Ae'].get('s').t()
...
all_activity_pred = all_activity(spike_record, assignments, 10)
proportion_pred = proportion_weighting(spike_record, assignments, proportions, 10)
print ('All activity accuracy: ')
print (torch.sum(labels[0:n_test].long() == all_activity_pred).item() / n_test)
print ('Proportion weighting accuracy: ')
print (torch.sum(labels[0:n_test].long() == proportion_pred).item() / n_test)
